    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-pink-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            {overview && overview.previousRevenue > 0 && (
              <div className={`flex items-center gap-1 text-sm ${revenueChange.isPositive ? 'text-green-200' : 'text-red-200'}`}>
                {revenueChange.isPositive ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
                {revenueChange.value.toFixed(1)}%
              </div>
            )}
          </div>
          <p className="text-3xl font-bold">{formatPrice(overview?.totalRevenue || 0)}</p>
          <p className="text-pink-100 text-sm mt-1">Total Revenue</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-4">
            ...
            {overview && overview.previousOrders > 0 && (
              <div className={`flex items-center gap-1 text-sm ${ordersChange.isPositive ? 'text-green-600' : 'text-red-600'}`}>
                {ordersChange.isPositive ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
                {ordersChange.value.toFixed(1)}%
              </div>
            )}
          </div>
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{overview?.totalOrders || 0}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{formatPrice(overview?.avgOrderValue || 0)}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{overview?.totalCustomers || 0}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
            ...
            <span className="text-sm text-gray-500">{getDateRangeLabel(dateRange)}</span>
          </div>
          <div className="p-4">
                ...
                {topProducts.map((product, index) => (
                  <div key={product.product_id} className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-pink-600">{formatPrice(product.total_revenue)}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {lowStockProducts.length > 0 ? (
              <div className="space-y-3">
                {lowStockProducts.slice(0, 5).map((product) => (
                  <div key={product.variant_id} className="flex items-center justify-between p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 dark:text-white truncate">
                        {product.product_title}
                      </p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          ...
          <a href="#" className="text-sm text-pink-600 hover:underline flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-green-600 font-medium">Conversion</span>
          </div>
          <p className="text-2xl font-bold text-green-700">--</p>
          <p className="text-sm text-green-600">rate (coming soon)</p>
        </div>
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <Eye className="h-4 w-4 text-purple-600" />
            <span className="text-sm text-purple-600 font-medium">Views</span>
          </div>
          <p className="text-2xl font-bold text-purple-700">--</p>
          <p className="text-sm text-purple-600">page views (coming soon)</p>
        </div>
