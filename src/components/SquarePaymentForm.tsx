...
export default function SquarePaymentForm({
  ...
  const initializePayments = useCallback(async (config: { applicationId: string; locationId: string }) => {
    try {
      ...
      // On desktop without Apple Pay support, Square will show a QR code
      try {
        ...
        console.log('Apple Pay initialized (QR code on desktop, native on mobile)');
      } catch (appleErr) {
        console.log('Apple Pay not available:', appleErr);
        // Still show Apple Pay button - Square will handle QR code flow
        setIsApplePayAvailable(true);
      }

      // Try to initialize Google Pay - works on both desktop and mobile
      try {
        ...
        // Google Pay is available on desktop Chrome and mobile
        setGooglePay(googlePayInstance);
        setIsGooglePayAvailable(true);
        console.log('Google Pay initialized');
      } catch (googleErr) {
        console.log('Google Pay initialization error:', googleErr);
        // Still try to show Google Pay - it works in most browsers
        setIsGooglePayAvailable(true);
      }
    ...
    <div className="space-y-6">
      ...
      {!isLoading && (
        <div className="space-y-3">
          ...
          {/* Row 1: Apple Pay & Google Pay - Always show, Square handles availability */}
          <div className="flex gap-3">
            {/* Apple Pay - Works on iOS/Safari natively, shows QR code on desktop */}
            <button
              type="button"
              onClick={handleApplePay}
              disabled={isProcessing || !applePay}
            ...
            {/* Google Pay - Works on all platforms */}
            <button
              type="button"
              onClick={handleGooglePay}
              disabled={isProcessing || !googlePay}
